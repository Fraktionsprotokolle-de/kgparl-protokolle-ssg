...
<xsl:stylesheet
    ...
    <xsl:template match="tei:note[not(parent::tei:teiHeader)]">
        <xsl:element name="a">
    ...
    <xsl:template match="tei:table">
        <xsl:element name="table">
            <xsl:attribute name="class">
                <xsl:text>kgparl-table</xsl:text>
            </xsl:attribute>
    ...
    <xsl:template match="tei:name[@type='Person']">
        <span class="persons entity {substring-after(@ref, '#')}" id="{@ref}" data-toggle="modal"
            data-target="{@ref}">
            <xsl:apply-templates />
        </span>
    </xsl:template>
    <xsl:template match="tei:rs">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="@type='person'">
                        <span class="persons entity {substring-after(@rendition, '#')}" id="{@ref}"
                            data-toggle="modal" data-target="{@ref}">
                            <xsl:apply-templates />
                        </span>
                    </xsl:when>
                    <xsl:when test="@type='place'">
                        <span class="places entity {substring-after(@rendition, '#')}"
                            id="{@xml:id}" data-toggle="modal" data-target="{@ref}">
                            <xsl:apply-templates />
                        </span>
                    </xsl:when>
                    <xsl:when test="@type='bibl'">
    ...
    <xsl:template match="tei:place">
        ...
        <div class="kgparl-modal" id="{@xml:id}" data-keyboard="false" tabindex="-1"
            aria-labelledby="{if(./tei:settlement) then(./tei:settlement/tei:placeName) else (./tei:placeName)}"
            aria-hidden="true">
            <div class="kgparl-modal-dialog">
                <div class="kgparl-modal-content">
                    <div class="kgparl-modal-header">
                        <h1 class="kgparl-modal-title" id="staticBackdropLabel">
                            <xsl:value-of
                                select="if(./tei:settlement) then(./tei:settlement/tei:placeName) else (./tei:placeName)" />
                        </h1>
                        <button type="button" class="modal-close-x" data-dismiss="modal"
                            aria-label="Schließen">×</button>
                    </div>
                    <div class="kgparl-modal-body">
                        <table>
                            <tbody>
                    ...
                    <div class="kgparl-modal-footer">
                        <button type="button" class="kgparl-btn-outline modal-close-btn" data-dismiss="modal">
                            Schließen</button>
                    </div>
                </div>
            </div>
