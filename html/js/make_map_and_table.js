...
function build_map_table(table_cfg) {
  ...
  // Accessibility: add aria-labels to Tabulator sort/page buttons
  table.on("tableBuilt", function () {
    addTabulatorAriaLabels(table.element);
  });
...
function addTabulatorAriaLabels(tableEl) {
  tableEl.querySelectorAll(".tabulator-col-sorter").forEach(function (btn) {
    var col = btn.closest(".tabulator-col");
    var title = col ? col.getAttribute("tabulator-field") || col.textContent.trim() : "";
    btn.setAttribute("aria-label", "Sortieren nach " + title);
  });
  tableEl.querySelectorAll(".tabulator-page").forEach(function (btn) {
    if (!btn.getAttribute("aria-label")) {
      var text = btn.textContent.trim();
      if (text === "First") btn.setAttribute("aria-label", "Erste Seite");
      else if (text === "Prev") btn.setAttribute("aria-label", "Vorherige Seite");
      else if (text === "Next") btn.setAttribute("aria-label", "Nächste Seite");
      else if (text === "Last") btn.setAttribute("aria-label", "Letzte Seite");
      else btn.setAttribute("aria-label", "Seite " + text);
    }
  });
}
